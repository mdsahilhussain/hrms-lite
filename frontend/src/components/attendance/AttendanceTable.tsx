...
export default function AttendanceTable({
  ...
  const totalPresent = attendanceList?.data?.filter(
    (a: any) => a.status === "Present"
  ).length;

  console.log("Filtered Attendance Data:", filteredData);
  console.log("Total Present Days:", attendanceList);
    ...
    <Card className="mt-8">
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-sm">
            {`View attendance records of ${selectedEmployee}. Use the filter to narrow down by date.`}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10">
          ...
          <p className="text-gray-500 bg-red-100 w-full">
        ...
        <Table headers={["Date", "Employee", "Status"]}>
